...
def train(args):
    ...
    if mode == 'train':
        ...
        loader_train = DataLoader(dataset_train, batch_size=batch_size, shuffle=True, num_workers=8)   # 더욱 빨라져!
    ...
    if mode == 'train':
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            loss_G_train = []
            loss_D_real_train = []
            loss_D_fake_train = []    # 여기 다 추가

            for batch, data in enumerate(loader_train, 1):
                # forward pass
                label = data['label'].to(device)
                input = torch.randn(label.shape[0], 100, 1, 1,).to(device)    #우리는 여기 지난번이랑 유지

                output = netG(input)

                # backward netD    두 가지 네트워크. 둘 다 해야한다
                set_requires_grad(netD, True)
                optimD.zero_grad()

                pred_real = netD(label)    # 라벨 넣을 때 true 인식
                pred_fake = netD(output.detach())    # output false
                ...
                pred_fake = netD(output)    # 여긴 그냥 사용
                ...
                if batch % 20 == 0:    # 세이브 하는 곳
                  # Tensorboard 저장하기    우리는 tonumpy(input) 남겨야해
                  output = fn_tonumpy(fn_denorm(output, mean=0.5, std=0.5)).squeeze()    # 0 ~ 1 사이라 디노말라이제이션
...
def test(args):
    ...
    if mode == "test":
        ...
        with torch.no_grad():    # Discriminator는 필요 없다!
            netG.eval()
